package defpackage;

import java.io.IOException;
import kotlin.reflect.jvm.internal.impl.protobuf.InvalidProtocolBufferException;

/* JADX INFO: loaded from: classes.dex */
public final class zrd extends u28 {
    public static final zrd K;
    public static final sk9 L = new sk9(6);
    public final l22 E;
    public int F;
    public int G;
    public yrd H;
    public byte I;
    public int J;

    static {
        zrd zrdVar = new zrd();
        K = zrdVar;
        zrdVar.G = 0;
        zrdVar.H = yrd.T;
    }

    public zrd(t34 t34Var, oc7 oc7Var) {
        wrd wrdVarH;
        this.I = (byte) -1;
        this.J = -1;
        boolean z = false;
        this.G = 0;
        this.H = yrd.T;
        j22 j22Var = new j22();
        v34 v34VarJ = v34.j(j22Var, 1);
        while (!z) {
            try {
                try {
                    try {
                        int iN = t34Var.n();
                        if (iN != 0) {
                            if (iN == 8) {
                                this.F |= 1;
                                this.G = t34Var.k();
                            } else if (iN == 18) {
                                if ((this.F & 2) == 2) {
                                    yrd yrdVar = this.H;
                                    yrdVar.getClass();
                                    wrdVarH = wrd.h();
                                    wrdVarH.i(yrdVar);
                                } else {
                                    wrdVarH = null;
                                }
                                yrd yrdVar2 = (yrd) t34Var.g(yrd.U, oc7Var);
                                this.H = yrdVar2;
                                if (wrdVarH != null) {
                                    wrdVarH.i(yrdVar2);
                                    this.H = wrdVarH.g();
                                }
                                this.F |= 2;
                            } else if (!t34Var.q(iN, v34VarJ)) {
                            }
                        }
                        z = true;
                    } catch (InvalidProtocolBufferException e) {
                        e.E = this;
                        throw e;
                    }
                } catch (IOException e2) {
                    InvalidProtocolBufferException invalidProtocolBufferException = new InvalidProtocolBufferException(e2.getMessage());
                    invalidProtocolBufferException.E = this;
                    throw invalidProtocolBufferException;
                }
            } catch (Throwable th) {
                try {
                    v34VarJ.k();
                } catch (IOException unused) {
                } catch (Throwable th2) {
                    this.E = j22Var.e();
                    throw th2;
                }
                this.E = j22Var.e();
                throw th;
            }
        }
        try {
            v34VarJ.k();
        } catch (IOException unused2) {
        } catch (Throwable th3) {
            this.E = j22Var.e();
            throw th3;
        }
        this.E = j22Var.e();
    }

    @Override // defpackage.aib
    public final boolean b() {
        byte b = this.I;
        if (b == 1) {
            return true;
        }
        if (b == 0) {
            return false;
        }
        int i = this.F;
        if ((i & 1) != 1) {
            this.I = (byte) 0;
            return false;
        }
        if ((i & 2) != 2) {
            this.I = (byte) 0;
            return false;
        }
        if (this.H.b()) {
            this.I = (byte) 1;
            return true;
        }
        this.I = (byte) 0;
        return false;
    }

    @Override // defpackage.b3
    public final int c() {
        int i = this.J;
        if (i != -1) {
            return i;
        }
        int iB = (this.F & 1) == 1 ? v34.b(1, this.G) : 0;
        if ((this.F & 2) == 2) {
            iB += v34.d(2, this.H);
        }
        int size = this.E.size() + iB;
        this.J = size;
        return size;
    }

    @Override // defpackage.b3
    public final i28 d() {
        return vrd.g();
    }

    @Override // defpackage.b3
    public final i28 e() {
        vrd vrdVarG = vrd.g();
        vrdVarG.i(this);
        return vrdVarG;
    }

    @Override // defpackage.b3
    public final void f(v34 v34Var) throws IOException {
        c();
        if ((this.F & 1) == 1) {
            v34Var.m(1, this.G);
        }
        if ((this.F & 2) == 2) {
            v34Var.o(2, this.H);
        }
        v34Var.r(this.E);
    }

    public zrd() {
        this.I = (byte) -1;
        this.J = -1;
        this.E = l22.E;
    }

    public zrd(vrd vrdVar) {
        this.I = (byte) -1;
        this.J = -1;
        this.E = vrdVar.E;
    }
}
