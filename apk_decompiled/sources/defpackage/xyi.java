...
public final class xyi {
    ...
    static {
        float[] fArr = usj.c;
        float fM = (float) ((((double) usj.m()) * 63.66197723675813d) / 100.0d);
        float[][] fArr2 = usj.a;
        float f = fArr[0];
        float[] fArr3 = fArr2[0];
        float f2 = fArr3[0] * f;
        ...
        if (d > 1.0d) {
            ...
            fExp = MTTypesetterKt.kLineSkipLimitMultiplier;
        }
        float[] fArr6 = {(((100.0f / f6) * fExp) + 1.0f) - fExp, (((100.0f / f7) * fExp) + 1.0f) - fExp, (((100.0f / f8) * fExp) + 1.0f) - fExp};
        float f9 = 1.0f / ((5.0f * fM) + 1.0f);
        float f10 = f9 * f9 * f9 * f9;
        float f11 = 1.0f - f10;
        float fCbrt = (0.1f * f11 * f11 * ((float) Math.cbrt(((double) fM) * 5.0d))) + (f10 * fM);
        float fM2 = usj.m() / fArr[1];
        double d2 = fM2;
        ...
        k = new xyi(fM2, ((fArr8[2] * 0.05f) + (fArr8[0] * 2.0f) + fArr8[1]) * fPow, fPow, fPow, 0.69f, 1.0f, fArr6, fCbrt, (float) Math.pow(fCbrt, 0.25d), fSqrt);
    }

    public xyi(float f, float f2, float f3, float f4, float f5, float f6, float[] fArr, float f7, float f8, float f9) {
        this.f = f;
        this.a = f2;
        this.b = f3;
        this.c = f4;
